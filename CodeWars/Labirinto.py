...
def movimento(posicao: tuple, direcao: list) -> list:
    ...
    """
    #posição do caminho que ele andou vai ser marcado com '2' = caminho percorrido
    LABIRINTO[posicao[0]][posicao[1]] = CAMINHO_PERCORRIDO
    #posição que ele está será marcado com '4' = robo
    LABIRINTO[posicao[0] + direcao[0]][posicao[1] + direcao[1]] = ROBO
    #retorna nova posiçao
...
def main():
    ...
    POSICAO_INICIAL = list(map(int, input("Digite a posição do robozinho (linha, coluna): ").split(',')))
    
    # verificar se a posição inicial é valida (se é diferente da parede ou saida) então coloca o robozinho
    while (LABIRINTO[POSICAO_INICIAL[0]][POSICAO_INICIAL[1]] == PAREDE) or (LABIRINTO[POSICAO_INICIAL[0]][POSICAO_INICIAL[1]] == SAIDA):
        POSICAO_INICIAL = list(map(int, input("Digite uma posição valida (linha, coluna): ").split(',')))
        
    #define o local da saida
    LABIRINTO[POSICAO_SAIDA[0]][POSICAO_SAIDA[1]] = SAIDA

    #posição atual recebe a posição inicial
    POSICAO_ATUAL = POSICAO_INICIAL 
    
    #imprimir labirinto com a posição do robo e a posição de saida
    print_labirinto()

    # contador para saber quantas posições o robo percorreu, se atingir 4 ele tentou todas posições
    contador_direcoes = 0 
